#!/usr/bin/env python3
"""
Multi-tracker evaluation on the stitched-walk blueberry MOT dataset with appearance-based trackers.
This variant enables appearance matching where supported.
"""
...
class TrackerEvaluator:
    ...
    def __init__(self, data_root: str, yolo_weights: str, reid_path: str, tracker_config_dir: str, output_dir: str):
        self.data_root = Path(data_root)
        self.yolo_weights = Path(yolo_weights)
        self.output_dir = Path(output_dir)
        self.tracker_config_dir = Path(tracker_config_dir)
        self.reid_weights = Path(reid_path)
    ...
    def _build_tracker(self, tracker_name: str):
        ...
        reid_weights = self.reid_weights if tracker_name in self.REID_TRACKERS else None
        ...
        if tracker_name == "deepocsort":
            ...
            tracker_args["metric"] = "res_recons"
            tracker_args["two_round_off"] = True
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-Tracker Evaluation on stitched-walk blueberry MOT Dataset with appearance models"
    )
    parser.add_argument(
        "--data_root",
        type=str,
    ...
    parser.add_argument("--reid_path", type=str, default="weights/osnet_ain_x1_0_blueberry.pt")
    parser.add_argument("--tracker_config_dir", type=str, default="configs/trackers")
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(REPO_ROOT / "output" / "eval" / "blueberry_mot_stitched_walk_reid"),
