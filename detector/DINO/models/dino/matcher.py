...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # Validate and auto-adjust target labels if needed
        # COCO datasets often use 1-indexed labels (1, 2, 3, ...) but models expect 0-indexed (0, 1, 2, ...)
        num_classes = out_prob.shape[1]
        if len(tgt_ids) > 0:
            max_tgt_id = tgt_ids.max().item()
            min_tgt_id = tgt_ids.min().item()
            
            # Final validation
            if max_tgt_id >= num_classes or min_tgt_id < 0:
                raise ValueError(
                    f"Target label indices out of range: min={min_tgt_id}, max={max_tgt_id}, "
                    f"num_classes={num_classes}. Check your dataset labels."
                )
        ...
        # Validate bbox shapes before cdist
        if len(tgt_bbox) > 0:
            if out_bbox.shape[1] != 4 or tgt_bbox.shape[1] != 4:
                raise ValueError(
                    f"Bbox shape mismatch: out_bbox={out_bbox.shape}, tgt_bbox={tgt_bbox.shape}. "
                    f"Expected shape [N, 4] for both."
                )
...
class SimpleMinsumMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if len(tgt_ids) > 0:
            ...
            # Auto-adjust: if labels are 1-indexed (min=1, max<=num_classes), convert to 0-indexed
            if min_tgt_id >= 1 and max_tgt_id <= num_classes:
                tgt_ids = tgt_ids - 1
                max_tgt_id = tgt_ids.max().item()
                min_tgt_id = tgt_ids.min().item()
