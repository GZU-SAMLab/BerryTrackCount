    ...
    const int im2col_step)
{
    ...
    AT_ASSERTM(level_start_index.size(0) == num_levels, 
               "level_start_index size (%ld) must match num_levels (%d)", 
               level_start_index.size(0), num_levels);
    AT_ASSERTM(spatial_shapes.size(0) == num_levels, 
               "spatial_shapes size (%ld) must match num_levels (%d)", 
               spatial_shapes.size(0), num_levels);
    
    // Validate spatial_size matches sum of all level spatial sizes
    int64_t total_spatial = 0;
    auto spatial_shapes_cpu = spatial_shapes.cpu();
    auto spatial_shapes_ptr = spatial_shapes_cpu.data_ptr<int64_t>();
    for (int i = 0; i < num_levels; ++i) {
        int64_t h = spatial_shapes_ptr[i * 2];
        int64_t w = spatial_shapes_ptr[i * 2 + 1];
        total_spatial += h * w;
    }
    AT_ASSERTM(total_spatial == spatial_size, 
               "spatial_size (%d) must equal sum of all level spatial sizes (%ld). "
               "Check that value tensor shape matches spatial_shapes.", 
               spatial_size, total_spatial);
