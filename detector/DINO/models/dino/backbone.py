    ...
    def __init__(self, name: str,
        ...
        if name in ['resnet18', 'resnet34', 'resnet50', 'resnet101']:
            ...
            try:
                ...
                # Import weights enum dynamically
                model_weights = getattr(torchvision.models, f'{name.capitalize()}_Weights', None)
                if model_weights is not None:
                    weights = model_weights.IMAGENET1K_V1 if use_pretrained else None
                    backbone = getattr(torchvision.models, name)(
                        replace_stride_with_dilation=[False, False, dilation],
                        weights=weights, norm_layer=batch_norm)
                else:
                    # Fallback to old API if weights enum not available
                    backbone = getattr(torchvision.models, name)(
                        replace_stride_with_dilation=[False, False, dilation],
                        pretrained=use_pretrained, norm_layer=batch_norm)
